        ...
        <x-container-secondary>
            <div class="grid grid-cols-12 bg-gray-900 rounded-3xl text-white">
                <div class="col-span-8 border-transparent border-b-black border-r-black border-2 w-auto p-2 ">
                    <textarea type="text" name="title" id="title" rows="1" maxlength="220"
                    class="form-control flex items-center justify-center autoresize w-full min-h-2 h-full resize-none border text-xl text-center border-none rounded-3xl px-2 r-2 py-2 bg-transparent placeholder-white align-middle" placeholder="Titulo">{{ old('title') }}</textarea>
                    @error('title')
                        <small class="text-red-600">{{ $message }}</small>
                    @enderror
                </div>
                  <div id="Importancia" class="col-span-4 border-transparent rounded-tr-3xl bg-red-600 border-b-black border-2 w-auto p-1">
                    <select id="value" class="flex items-center text-center justify-center text-xl bg-red-600 m-0 w-full h-full border-gray-300 dark:border-none dark:text-white focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-3xl shadow-sm" type="number" name="value" :value="old('value')" autocomplete="value">
                  ...
                  <div class="col-span-12 border-transparent bg-gray-600 border-b-black border-2 w-auto p-2">
                    <textarea type="text" name="content" id="content" maxlength="700"
                    class="form-control flex items-center justify-center autoresize w-full min-h-10 h-44 resize-none border text-xl text-center border-none rounded-3xl px-2 r-2 py-1 bg-transparent placeholder-white" placeholder="Contenido">{{ old('content') }}</textarea>
                    @error('content')
                        <small class="text-red-600">{{ $message }}</small>
                    @enderror
                  </div>
                  <div class="col-span-6 text-black text-right border border-transparent w-auto p-2">
                    <input class="text-xl h-20 rounded-3xl bg-cyan-600 text-white" type="date" id="date" name="date" value="{{ old('date') }}">
                    @error('date')
                        <br>
                        <small class="text-red-600">{{ $message }}</small>
                    @enderror
                  ...
                  <div class="col-span-6 text-black text-left border border-transparent w-auto p-2">
                    <input class="text-xl h-20 rounded-3xl bg-cyan-600 text-white" type="time" id="time" name="time" value="{{ old('time') }}">
                    @error('time')
                        <br>
                        <small class="text-red-600">{{ $message }}</small>
                    @enderror
